...
export const optimizeMarketingTopic = async (userTopic: string) => {
    ...
    try {
      const data = await callOpenAI({
        ...
        messages: [
          ...
          사용자가 입력한 주제 "${userTopic}"와 매우 밀접하게 연관된 5가지 최적화 주제를 추천하세요.
          추천하는 주제는 반드시 사용자의 초기 의도를 벗어나지 않으면서도 검색 노출에 유리해야 합니다.
          '2026년'이라는 단어를 직접적으로 너무 자주 언급하기보다는, 현재가 2026년임을 전제로 한 최신 트렌드를 자연스럽게 반영해주세요.
          
          [분석 가이드라인]
          1. **주제 연관성**: 모든 추천 주제는 반드시 "${userTopic}"의 핵심 가치와 연관되어야 합니다.
          2. **검색 의도 분석**: 정보성, 상업성, 탐색성 의도를 구분하여 제안하세요.
          3. **롱테일 키워드 전략**: 경쟁은 낮고 전환율은 높은 구체적인 주제를 포함하세요.
          4. **타겟 세분화**: 연령, 성별, 관심사뿐만 아니라 '고민 지점(Pain Point)'을 명시하세요.
  ...
  const response = await callGemini(ai, 'gemini-3-flash-preview', {
    ...
    사용자가 입력한 주제 "${userTopic}"와 매우 밀접하게 연관된 5가지 최적화 주제를 추천하세요.
    모든 추천은 사용자의 초기 의도를 존중하며, 클릭률(CTR)과 체류 시간을 극대화할 수 있어야 합니다.
    '2026년'이라는 단어의 직접적인 반복은 피하되, 최신성을 충분히 반영한 자연스러운 서술을 사용하세요.
    
    [분석 가이드라인]
    1. **주제 연관성**: 모든 추천 주제는 반드시 "${userTopic}"의 핵심 키워드와 맥락을 유지해야 합니다.
    2. **검색 의도 분석**: 정보성, 상업성, 탐색성 의도를 구분하여 제안하세요.
    3. **롱테일 키워드 전략**: 경쟁은 낮고 전환율은 높은 구체적인 주제를 포함하세요.
    4. **타겟 세분화**: 연령, 성별, 관심사뿐만 아니라 '고민 지점(Pain Point)'을 명시하세요.
...
export const generateGoldenKeywords = async (topic: string, target: string) => {
    ...
    try {
      const data = await callOpenAI({
        ...
        messages: [
          ...
          주제 "${topic}"와 직접적으로 연관된, 2026년 현재 검색량은 많지만 경쟁은 적은 '황금 키워드' 20개를 추출하세요.
          모든 키워드는 반드시 주제 "${topic}"의 맥락 안에서 유효해야 합니다.
          
          [키워드 선정 기준]
          1. **주제 밀착성**: 추천되는 모든 키워드는 사용자가 입력한 주제 "${topic}"와 연관성이 매우 높아야 합니다.
          2. **LSI 키워드**: 2026년의 언어 사용 트렌드와 주제와 연관된 의미적 키워드를 포함하여 SEO 점수를 높이세요.
          3. **구매 여정 단계**: 인지, 고려, 결정 단계별 키워드를 고르게 배치하세요.
          4. **데이터 추정**: 2026년 기준 검색량(Volume), 경쟁 정도(Competition), 수익성(Profitability)을 상/중/하로 평가하세요.
  ...
  const response = await callGemini(ai, 'gemini-3-flash-preview', {
    ...
    주제 "${topic}"와 매우 밀접하게 연관된, 2026년 현재 검색량은 많지만 경쟁은 적은 '황금 키워드' 20개를 추출하고 메타데이터를 포함하세요. 
    모든 키워드는 주제의 맥락을 벗어나지 않아야 하며, 2026년의 LSI 키워드와 구매 여정 단계를 고려하세요.
    결과는 JSON 형식이어야 합니다.`,
    config: {
      responseMimeType: "application/json",
      responseSchema: {
...
export const generateBlogPost = async (title: string, keywords: string[], style: string, target: string, topic: string = "") => {
  checkAIAvailability();
  const openaiKey = getOpenAIKey();
  if (openaiKey) {
    try {
      const data = await callOpenAI({
        ...
        messages: [
          ...
          [SEO 및 품질 최적화 규칙]
          1. **주제 연관성**: 글의 모든 내용은 반드시 주제 "${topic || title}"와 밀접하게 연관되어야 하며, 독자에게 실질적인 가치를 제공해야 합니다.
          2. **전문성 및 신뢰성(E-E-A-T)**: 2026년 현재 해당 분야의 20년 경력 전문가가 작성한 것처럼 깊이 있는 통찰력과 구체적인 사례를 포함하세요.
          3. **최신성 유지**: 2026년의 최신 트렌드와 정보를 반영하되, '2026년'이라는 단어의 직접적인 반복은 피하고 자연스럽게 현재가 2026년임을 알 수 있도록 서술하세요.
          4. **글의 구조**: 
             - 서론: 독자의 문제점을 공감하고 해결책을 제시하는 강력한 훅(Hook)으로 시작하세요. 반드시 소제목을 포함하세요.
             - 본론: 정보성 가치가 높은 내용을 3개의 소제목(##)으로 나누어 상세히 설명하세요.
             - 결론: 핵심 내용을 요약하고 독자의 행동을 유도하는 강력한 CTA(Call to Action)를 포함하세요. 반드시 소제목을 포함하세요.
             - **주의**: '서론', '본론', '결론'이라는 단어를 제목이나 텍스트로 직접 작성하지 마세요.
          5. **소제목 및 가독성**:
             - 소제목(##)은 짧고 간단하며 임팩트 있게 작성하세요.
             - 소제목 아래의 본문은 모든 글자를 진하게 하지 마세요.
             - **중요한 키워드나 강조하고 싶은 단어에만** **강조** 기호를 사용하여 강조하세요.
             - 가독성을 위해 **매 문장이 끝나면 반드시 줄바꿈**을 시행하세요.
             - 한 문장은 짧고 명확하게 작성하세요.
             - 한 문단은 3줄을 넘지 않도록 구성하세요.
          6. **키워드 배치**: 선택된 키워드들을 서론, 소제목, 본문 내용에 자연스럽게 녹여내어 검색 엔진 최적화를 극대화하세요.
          7. **해시태그**: 2026년의 검색 트렌드를 반영한 최적의 해시태그를 반드시 20개 이상 생성하세요.
          8. **이미지 전략**: [IMAGE_PLACEHOLDER_1], [IMAGE_PLACEHOLDER_2], [IMAGE_PLACEHOLDER_3]를 서론 하단, 본론 중간, 결론 상단에 배치하세요.
          9. **이미지 프롬프트(imagePrompts)**: 2026년의 감각에 맞는 고품질 이미지 묘사를 영문으로 작성하세요. 
  ...
  const response = await callGemini(ai, 'gemini-3-flash-preview', {
    ...
    [SEO 및 품질 최적화 규칙]
    1. **주제 연관성**: 글의 모든 내용은 반드시 주제 "${topic || title}"와 밀접하게 연관되어야 합니다.
    2. **전문성 및 신뢰성(E-E-A-T)**: 2026년 현재의 전문가가 작성한 것처럼 깊이 있는 통찰력을 포함하세요.
    3. **최신성**: 2026년의 최신 트렌드를 반영하되, '2026년' 단어의 과도한 반복은 피하고 자연스럽게 서술하세요.
    4. **글의 구조**: 서론(훅), 본론(3개의 소제목), 결론(CTA) 구조를 엄격히 지키되, 각 섹션에 반드시 소제목(##)을 포함하세요. '서론', '본론', '결론'이라는 단어를 직접 작성하지 마세요.
    5. **소제목 및 가독성**: 소제목(##)은 짧고 간단하게 작성하고, 본문은 중요한 단어에만 **강조**를 사용하세요. 매 문장이 끝나면 반드시 줄바꿈을 하세요. 문장은 짧게, 문단은 3줄 이내로 작성하세요.
    6. **키워드 배치**: 키워드들을 자연스럽게 문맥에 녹여내세요.
    7. **해시태그**: 2026년 트렌드에 맞는 해시태그를 반드시 20개 이상 생성하세요.
    8. **이미지 전략**: [IMAGE_PLACEHOLDER_1, 2, 3]을 적절한 위치에 배치하세요.
    9. **이미지 프롬프트**: 2026년 감각의 "High-end professional photography", "Modern Korean lifestyle" 스타일 영문 프롬프트를 생성하세요.`,
    config: {
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.OBJECT,
...
export const getGoldenTopicRecommendations = async (category: string, topic: string) => {
  ...
  해당 카테고리 내에서 사용자가 입력한 주제 "${topic}"와 매우 밀접하게 연관된 구체적인 블로그 주제 5개를 추천해주세요. 
  모든 추천 주제는 사용자의 초기 의도를 존중하며, 검색 유입이 높고 체류 시간이 길어질 수 있는 내용이어야 합니다.
...
export const getGoldenPersonaRecommendations = async (category: string, topic: string) => {
  ...
  주제 "${topic}"에 대해 가장 큰 관심을 가질만한 구체적인 타겟 독자(페르소나) 5명을 추천해주세요. 
  추천하는 페르소나는 반드시 해당 주제와 실질적인 연관성이 있어야 합니다.
...
export const getGoldenKeywordRecommendations = async (category: string, topic: string, persona: string) => {
  ...
  위 정보를 종합 분석하여 주제 "${topic}"와 매우 밀접하게 연관된, 검색 노출에 유리한 황금 키워드를 추천해주세요. 
  모든 키워드는 반드시 주제의 맥락 안에서 유효해야 하며, 주요 키워드, 보조 키워드, 롱테일 키워드를 포함해야 합니다.
  
  [분석 기준]
  - 주제 밀착성(가장 중요), 검색 의도 적합성, 경쟁 강도, 예상 검색량, 수익성, 한국 사용자 친화성 등
  
  [제외 규칙]
  - 주제와 무관한 키워드, 일반적인 단일 단어, 특정 브랜드명, 비속어, 경쟁 과도 키워드 제외
...
export const generateGoldenBlogPost = async (data: {
  ...
  [필수 규칙]
  1. **주제 연관성**: 글의 모든 내용은 반드시 주제 "${data.topic}"와 매우 밀접하게 연관되어야 합니다. 주제를 벗어난 내용은 포함하지 마세요.
  2. 자연스럽고 사람처럼 읽히는 문체 (AI 느낌 제거)
  3. 분량: 1500~2000자
  4. 구조: 제목, 서론(300-400자), 본론(800-1200자), 결론(300-400자), 콜투액션, 해시태그(7-10개)
  5. 가독성: 짧고 명료한 문장, 소제목 적극 사용, 핵심 정보 상단 배치
  6. 키워드 배치: 핵심 키워드 2-3개, 보조 키워드 5-7개를 제목, 서론, 소제목, 결론에 자연스럽게 배치 (스터핑 금지)
  7. 언어: 자연스러운 한국어 (번역투 금지, 맞춤법 준수)
  8. 신뢰성: 사실 기반 정보, 불확실한 정보 단정 금지, 구체적 상업시설명 언급 금지
  9. 이미지 배치: [IMAGE_PLACEHOLDER_1] (서론), [IMAGE_PLACEHOLDER_2] (본론), [IMAGE_PLACEHOLDER_3] (결론)
  10. 이미지 프롬프트: 각 이미지 위치에 맞는 고품질 실사 이미지 묘사 (영문으로 작성, 한국인이 등장할 경우 "Korean person" 명시)
